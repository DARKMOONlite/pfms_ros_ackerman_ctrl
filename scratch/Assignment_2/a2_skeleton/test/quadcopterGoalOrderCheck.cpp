...
Mission mission(controllers);
   ...
   ASSERT_EQ(ordered_goals.at(2).x,goals.at(1).x);
ASSERT_EQ(ordered_goals.at(0).x,goals.at(2).x);
ASSERT_EQ(ordered_goals.at(4).x,goals.at(3).x);
ASSERT_EQ(ordered_goals.at(5).x,goals.at(4).x);
ASSERT_EQ(ordered_goals.at(9).x,goals.at(5).x);
ASSERT_EQ(ordered_goals.at(8).x,goals.at(6).x);
ASSERT_EQ(ordered_goals.at(7).x,goals.at(7).x);
ASSERT_EQ(ordered_goals.at(6).x,goals.at(8).x);
ASSERT_EQ(ordered_goals.at(1).x,goals.at(9).x);

}

TEST(CheckGoalOrder, Set2){
 LinkCommand* linkCommand = new LinkCommand(true);
    {
        Odometry odo = populateOdoUGV(0,2,0);
        linkCommand->writeCommand(odo);
...
for(int i=0; i<9; i++){
    pfms::geometry_msgs::Point test{i+1,i+1};
    goals.push_back(test);
}
...
ASSERT_EQ(ordered_goals.at(2).x,goals.at(2).x);
ASSERT_EQ(ordered_goals.at(3).x,goals.at(3).x);
ASSERT_EQ(ordered_goals.at(4).x,goals.at(4).x);
ASSERT_EQ(ordered_goals.at(5).x,goals.at(5).x);
ASSERT_EQ(ordered_goals.at(6).x,goals.at(6).x);
ASSERT_EQ(ordered_goals.at(7).x,goals.at(7).x);
ASSERT_EQ(ordered_goals.at(8).x,goals.at(8).x);
