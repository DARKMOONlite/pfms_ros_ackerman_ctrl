...
TEST(RangeProcessing,DetectHeightOfObject){
  ...
  path += "/test/bag/";
  std::string file = path + "sample1.bag";
  ...
  double height=0.0;
  bool objectDetected = range_processing::detectHeightOfObject(*range, odom->pose.pose,height);


  EXPECT_TRUE(objectDetected);

  EXPECT_NEAR(height,4.5,0.1);
}
...
TEST(RangeProcessing,DetectPerson){
  ...
  EXPECT_NEAR(location.x,5.01,0.1);
  EXPECT_NEAR(location.y,6.30,0.1);
  EXPECT_NEAR(location.z,1.86,0.1);
}
