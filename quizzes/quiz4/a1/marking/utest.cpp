...
TEST(QuadcopterTest, Goals) {
    ...
    // We try to reach fisrt point
    {
        ...
        double actualDistance = std::pow (std::pow(pt.x-0,2)+std::pow(pt.y-0,2),0.5);

        ASSERT_TRUE(reachable);// The goal should be reachable
        ASSERT_NEAR(dist,actualDistance,tolerance);//Distance 4.123[m]
        ASSERT_NEAR(t,actualDistance/0.4,0.5); //Time 3.535 [s], tolerance 0.5 [s] for the answer
    ...
    // If we did not reach first goal.
    {
        ...
        double actualDistance = std::pow (std::pow(pt.x-4,2)+std::pow(pt.y-1,2),0.5);

        ASSERT_TRUE(reachable);
        ASSERT_NEAR(dist,actualDistance,tolerance);
        ASSERT_NEAR(t,actualDistance/0.4,0.5);
